...
uses
  Classes, SysUtils, strutils, LazUTF8, lclintf, FileUtil, Forms, Controls, Graphics, Dialogs, StdCtrls;
...
var
  cnt, i : Integer;
  //n:=0: Integer; // идентификатор в массиве.
  //arr: Array of Integer; // для записи номеров удаляемых строк.
begin
  ...
  for i:=0 to cnt do // цикл по массиву TMemo с нуля, а номера строк с 1.
    begin
      if CompareStr('+', LeftStr(Memo1.Lines[i], 1)) = 0 then
          //begin
          //  arr[n]:=i; n:=n+1;// порядковый номер следующего.
          //end;
...
begin
     Memo1.Clear;
     S := 'Привет родной, любимый мой, ты уже молодец!'+LineEnding+'Я рад тебе и верю в тебя. Сейчас, здесь.'+LineEnding+'Ты очень нужен мне. Нужен. Любой.'+LineEnding+LineEnding+'У тебя получится всё чему должно получиться. Обязательно!'+LineEnding+'Ты же такой умничка!'+LineEnding+'Обнимаю тебя, мой родной.'+LineEnding;
     Memo1.Text:= S;
end;
